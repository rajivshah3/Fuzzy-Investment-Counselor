...
def main():
    ...
    train_again = True
    do_suggest_weights = False
    n_smoothing_period = 14

    apply_moving_average = True
    movingAverage_windowSize_list = [200]

    initial_day_in_testSet = 300
    target_day_range = 3
    window_size_covariance_calculation = 100
    window_size_of_observation_for_prediction_list = [5] #range(5, 45, 5)
    c_list = [1000]#[0.1, 10, 100, 1000]
    gamma_list = [0.001]#[0.0001, 0.001, 0.01, 0.1]
    ...
    for movingAverage_windowSize in movingAverage_windowSize_list:
        print('-------------------Window:', movingAverage_windowSize )
        for c in c_list:
            for gamma in gamma_list:
                for window_size_of_observation_for_prediction in window_size_of_observation_for_prediction_list:
                    mutual_name = '_Window' + str(window_size_of_observation_for_prediction) + '_c' + str(c) + '_gamma' + str(gamma)
                    ...
                    for stock_index in range(len(stocks_to_process)):
                        ...
                        AG[n_smoothing_period] = np.mean(Gain_closing[
                                                         0:n_smoothing_period])  # np.max([np.mean(Gain_closing[0:n_smoothing_period]), 0.00001])
                        AL[n_smoothing_period] = np.mean(Loss_closing[
                                                         0:n_smoothing_period])  # np.max([np.mean(Loss_closing[0:n_smoothing_period]), 0.00001])
                        for day_index in range(n_smoothing_period + 1, total_number_of_days):
                            AG[day_index] = (AG[day_index - 1] * (n_smoothing_period - 1) + Gain_closing[
                                day_index]) / n_smoothing_period
                            AL[day_index] = (AL[day_index - 1] * (n_smoothing_period - 1) + Loss_closing[
                                day_index]) / n_smoothing_period
                        RS = AG / AL
                        RSI = 100 - (100 / (1 + RS))
                        ...
                        TR = np.maximum(highest - lowest, np.abs(highest - yesterday_closing),
                                        np.abs(lowest - yesterday_closing))
                        STR = np.zeros((total_number_of_days, 1))
                        STR[n_smoothing_period] = np.mean(
                            TR[0:n_smoothing_period])  # np.max([np.mean(TR[0:n_smoothing_period]), 0.00001])
                        PDM = np.maximum(highest - yesterday_highest, 0)
                        SPDM = np.zeros((total_number_of_days, 1))
                        SPDM[n_smoothing_period] = np.mean(
                            PDM[0:n_smoothing_period])  # np.max([np.mean(PDM[0:n_smoothing_period]), 0.00001])
                        NDM = np.maximum(yesterday_lowest - lowest, 0)
                        SNDM = np.zeros((total_number_of_days, 1))
                        SNDM[n_smoothing_period] = np.mean(
                            NDM[0:n_smoothing_period])  # np.max([np.mean(NDM[0:n_smoothing_period]), 0.00001])
                        for day_index in range(n_smoothing_period + 1, total_number_of_days):
                            STR[day_index] = (STR[day_index - 1] * (n_smoothing_period - 1) + TR[
                                day_index]) / n_smoothing_period
                            SPDM[day_index] = (SPDM[day_index - 1] * (n_smoothing_period - 1) + PDM[
                                day_index]) / n_smoothing_period
                            SNDM[day_index] = (SNDM[day_index - 1] * (n_smoothing_period - 1) + NDM[
                                day_index]) / n_smoothing_period
                        SPDI = 100 * SPDM / STR
                        SNDI = 100 * SNDM / STR
                        DX = 100 * np.abs(SPDI - SNDI) / (SPDI + SNDI)
                        ADX = np.zeros((total_number_of_days, 1))
                        ADX[2 * n_smoothing_period] = np.max(
                            [np.mean(DX[n_smoothing_period:2 * n_smoothing_period]), 0.00001])
                        for day_index in range(n_smoothing_period + 1, total_number_of_days):
                            ADX[day_index] = (ADX[day_index - 1] * (n_smoothing_period - 1) + DX[
                                day_index]) / n_smoothing_period
                        ...
                        training_set_not_averaged, testing_set_not_averaged = divide_dataset_to_training_and_testing_sets(
                        ...
                        if apply_moving_average:
                            ...
                            OpenningClosingLowHighVolume_firstDay_repeat = np.tile(OpenningClosingLowHighVolume_firstDay,
                                                                                   (movingAverage_windowSize - 1, 1))
                            OpenningClosingLowHighVolume_padded = np.vstack(
                                (OpenningClosingLowHighVolume_firstDay_repeat, OpenningClosingLowHighVolume))
                            ...
                            for column_index in range(OpenningClosingLowHighVolume_padded.shape[1]):
                                ...
                                time_series_MovingAveraged = moving_average(data_set=time_series.ravel(),
                                                                            periods=movingAverage_windowSize)
                            ...
                            # ------ splitting data into training and testing:
                            training_set_withValidationSet, testing_set = divide_dataset_to_training_and_testing_sets(
                                dataset=data_a_stock_averaged, fraction_of_training_set=fraction_of_training_set)
                            training_set, testing_set = divide_dataset_to_training_and_testing_sets(
                                dataset=training_set_withValidationSet,
                                fraction_of_training_set=0.8)  # here, second output is validation set
                        ...
                        for window_index in range(1, window_size_of_observation_for_prediction):
                            ...
                            Z_test[:window_index, :] = training_set[-window_index:, :]
                        ...
                        for stock_variable_index in range(number_of_stock_variables): # iterating over: Openning, Closing, Low, High, Volume
                            ...
                            if train_again:
                                if predictor_type == 'SVR':
                                    ...
                                    name_to_save = 'SVR_model_list_' + name_of_stock + '_StockVar' + str(stock_variable_index) + mutual_name
                                    ...
                                    name_of_file_to_save = 'LSTM_model_' + name_of_stock + '_StockVar' + str(stock_variable_index)  + mutual_name +  '.h5'
                                    ...
                                    name_to_save = 'LSTM_loss_history_' + name_of_stock + '_StockVar' + str(stock_variable_index)  + mutual_name
                                    save_data(data=history, name_to_save=name_to_save, path=path_save)
                            else:
                                if predictor_type == 'SVR':
                                    name_to_load = 'SVR_model_list_' + name_of_stock + '_StockVar' + str(stock_variable_index)  + mutual_name
                                    ...
                                    for stock_var_index_to_load in range(number_of_stock_variables):
                                        name_of_file_to_load = 'LSTM_model_' + name_of_stock + '_StockVar' + str(stock_variable_index)  + mutual_name + '.h5'
                            ...
                            if predictor_type == 'SVR':
                                ...
                                # print(LSTM_model_list[stock_variable_index])
                                # input('hi2')
                        ...
                        for stock_variable_index in range(number_of_stock_variables):  # iterating over: Openning, Closing, Low, High
                            ...
                            MAE_list.append(MAE)
                            name_to_save = 'Mean_Abs_Error_' + name_of_stock + mutual_name
                            ...
                            name_to_save = 'Coefficieent_of_Determinent_' + name_of_stock + mutual_name
                            ...
                            name_to_save = 'Root_Mean_Square_Error_' + name_of_stock + mutual_name
                            ...
                            if do_plot and stock_variable_index == predict_which_stock_variable:
                                name_of_image = 'predicted_dollor_' + name_of_stock + mutual_name
                                plot_time_series(time_series_list=[testing_set[:, stock_variable_index], predicted_testSet_AllStockVariables_unscaled[:, stock_variable_index]], legends=['Actual', 'Predicted'], path_save=path_save+name_of_stock+'/plots/', name_of_image=name_of_image, format_of_image='png')
                                name_of_image = 'predicted_percent_' + name_of_stock + mutual_name
                            ...
                            save_data(data=predicted_testSet_AllStockVariables_unscaled[:, stock_variable_index], name_to_save='predicted_testSet_StockVariable' + str(stock_variable_index) + '_unscaled' + mutual_name,
                            ...
                            save_np_array_to_txt(variable=np.array(predicted_testSet_AllStockVariables_unscaled[:, stock_variable_index]),
                                                 name_of_variable='predicted_testSet_StockVariable' + str(stock_variable_index) + '_unscaled' + mutual_name,
                                                 path_to_save=path_save + 'time_series_prediction/' + str(name_of_stock) + '/')
                            save_data(data=testing_set[:, stock_variable_index],
                                      name_to_save='testing_set_StockVariable' + str(stock_variable_index) + mutual_name,
                                      path=path_save + 'time_series_prediction/' + str(name_of_stock) + '/')
                            save_np_array_to_txt(variable=np.array(testing_set[:, stock_variable_index]),
                                                 name_of_variable='testing_set_StockVariable' + str(stock_variable_index) + mutual_name,
                                                 path_to_save=path_save + 'time_series_prediction/'  + str(name_of_stock) + '/')
                            save_data(data=actual_testSet_stockVariable_percentage,
                                      name_to_save='actual_testSet_stockVariable' + str(stock_variable_index) + '_percentage' + mutual_name,
                                      path=path_save + 'time_series_prediction/' + str(name_of_stock) + '/')
                            save_np_array_to_txt(variable=np.array(actual_testSet_stockVariable_percentage),
                                                 name_of_variable='actual_testSet_stockVariable' + str(stock_variable_index) + '_percentage' + mutual_name,
                                                 path_to_save=path_save + 'time_series_prediction/' + str(name_of_stock) + '/')
                            save_data(data=predicted_testSet_stockVariable_percentage,
                                      name_to_save='predicted_testSet_stockVariable' + str(stock_variable_index) + '_percentage' + mutual_name,
                                      path=path_save + 'time_series_prediction/' + str(name_of_stock) + '/')
                            save_np_array_to_txt(variable=np.array(predicted_testSet_stockVariable_percentage),
                                                 name_of_variable='predicted_testSet_stockVariable' + str(stock_variable_index) + '_percentage' + mutual_name,
                                                 path_to_save=path_save + 'time_series_prediction/' + str(name_of_stock) + '/')
                        ...
                        #     plot_candle(time_series_list=predicted_time_series_list, path_save=path_save+name_of_stock+'/plots/', name_of_image=name_of_image, format_of_image='png')
                        ...
                        actual_testingSet_AllStocks_targetStockVariable.append(testing_set[:, predict_which_stock_variable])
        ...
        save_np_array_to_txt(variable=hit_rate_average_over_stocks,
                             name_of_variable='hit_rate_average_over_stocks'+str(movingAverage_windowSize),
                             path_to_save=path_save + 'time_series_prediction/')
        save_np_array_to_txt(variable=hit_rate_all_stocks,
                             name_of_variable='hit_rate_all_stocks'+str(movingAverage_windowSize),
                             path_to_save=path_save + 'time_series_prediction/')
        save_np_array_to_txt(variable=MAE_all_stocks,
                             name_of_variable='MAE_all_stocks'+str(movingAverage_windowSize),
                             path_to_save=path_save + 'time_series_prediction/')
        save_np_array_to_txt(variable=RMSE_all_stocks,
                             name_of_variable='RMSE_all_stocks'+str(movingAverage_windowSize),
                             path_to_save=path_save + 'time_series_prediction/')
        save_np_array_to_txt(variable=hit_rate_std_over_stocks,
                             name_of_variable='hit_rate_std_over_stocks'+str(movingAverage_windowSize),
                             path_to_save=path_save + 'time_series_prediction/')
    if do_suggest_weights:
        ...
        while target_day_index <= target_day_range:
            ...
            print(actual_AllDataset_AllStocks_targetStockVariable_percentage[0].shape)
            print(actual_AllDataset_AllStocks_targetStockVariable_percentage[1].shape)
            print(actual_AllDataset_AllStocks_targetStockVariable_percentage[2].shape)
            #print(actual_AllDataset_AllStocks_targetStockVariable_percentage[3].shape)
            # input('hi')

            actual_AllDataset_AllStocks_targetStockVariable_percentage = np.asarray(actual_AllDataset_AllStocks_targetStockVariable_percentage)
            target_day_with_offset_of_trainingSet = (target_day_index - 1 + initial_day_in_testSet -1) + training_set.shape[0]
            stocks_information_for_covariance_calculation = actual_AllDataset_AllStocks_targetStockVariable_percentage[:,target_day_with_offset_of_trainingSet-window_size_covariance_calculation:target_day_with_offset_of_trainingSet]
            ...
            for stock_index in range(number_of_stocks):
                ...
                actual_tomorrow_percentage = np.vstack([actual_tomorrow_percentage, actual_testSet_AllStocks_targetStockVariable_percentage[stock_index].ravel()[target_day_index - 1 + initial_day_in_testSet - 1]])
            ...
            print('actual tomorrows prices:', list(actual_tomorrow_dollars.ravel()))
            ...
            #------ Reporting weights obtained by Fuzzy invest counselor:
            ...
            print('actual tomorrows prices:', list(actual_tomorrow_dollars.ravel()))
            print('portfolio weights:', random_weights)
        ...
        plot_color_between_lines(time_series_list=[actual_Budget_portfolio_list, predicted_Budget_portfolio_list, predicted_lower_bound_Budget_portfolio_list, predicted_upper_bound_Budget_portfolio_list,
                                                   actual_Budget_Fuzzy_list, predicted_Budget_Fuzzy_list, predicted_upper_bound_Budget_Fuzzy_list, predicted_lower_bound_Budget_Fuzzy_list, actual_Budget_random_list,
                                                   predicted_Budget_random_list, predicted_lower_bound_Budget_random_list, predicted_upper_bound_Budget_random_list],
        ...
        print('****************************')
        print('actual_Budget_Fuzzy', actual_Budget_portfolio_list)
        print('predicted_Budget_Fuzzy', predicted_Budget_portfolio_list)
        print('upper', predicted_upper_bound_Budget_portfolio_list)
        print('lower', predicted_lower_bound_Budget_portfolio_list)
        print('actual_Budget_Fuzzy', predicted_Budget_Fuzzy_list)
...
def plot_color_between_lines(time_series_list, path_save='./', name_of_image='img', format_of_image='png'):
    ...
    plt_fig[0], = plt.plot(range(len(time_series)), time_series,  '-', color='g', label='Actual budget based on portfolio')

    # Predicted based on portfolio:
    time_series = time_series_list[1]
    plt_fig[1], = plt.plot(range(len(time_series)), time_series, '--', color='k', label='Predicted budget based on portfolio')
    ...
    plt_fig[4], = plt.plot(range(len(time_series)), time_series, '-', color='b', label='Actual budget based on Fuzzy')

    # predicted of Fuzzy:
    time_series = time_series_list[5]
    plt_fig[5], = plt.plot(range(len(time_series)), time_series, '--', color='r', label='Predicted budget based on Fuzzy')
    ...
    plt.xticks(range(len(time_series)))
    plt.legend(handles=plt_fig)
    # plt.show()
    plt.savefig(path_save+name_of_image+'.'+format_of_image)
